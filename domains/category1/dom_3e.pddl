...
(:predicates ;todo: define predicates here
    ...
    (potential_connection ?r1 - track ?r2 - track)
    (blocked ?r - track)
...
(:durative-action switch_a
    ...
    :condition (and 
        (over all (not (blocked ?ra1)))
        (over all (not (blocked ?ra2)))
        (over all (not (blocked ?rb)))
        (over all (potential_connection ?ra1 ?rb))
        (over all (potential_connection ?ra2 ?rb))
        (at start (connected ?ra1 ?rb))
    )
    :effect (and 
        (at start (not (connected ?ra1 ?rb)))
        (at end (connected ?ra2 ?rb))
    )
)

(:durative-action switch_b
    :parameters (
        ?rb1 - track
        ?rb2 - track
        ?ra - track
    ...
    :condition (and 
        ; statics
        (at start (potential_connection ?ra ?rb1))
        (at start (potential_connection ?ra ?rb2))
        ; other
        (over all (not (blocked ?rb1)))
        (over all (not (blocked ?rb2)))
        (over all (not (blocked ?ra)))
        (at start (connected ?ra ?rb1))
    )
    :effect (and 
        (at start (not (connected ?ra ?rb1)))
        (at end (connected ?ra ?rb2))
    )
)
...
(:durative-action move_aside_empty
    ...
    :effect (and 
        ...
        (at end (available ?t))
        ; block tracks
        (at start (blocked ?from))
        (at start (blocked ?to))
        (at end (not (blocked ?from)))
        (at end (not (blocked ?to)))
    )
)

(:durative-action enter_and_move_aside_empty
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
...
(:durative-action enter_and_move_aside_occupied
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
...
(:durative-action enter_and_move_bside_empty
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
...
(:durative-action enter_and_move_bside_occupied
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
