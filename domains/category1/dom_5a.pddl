...
(:durative-action walk_and_enter_aside
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
    ...
    :duration (= ?duration (+ (walking_duration ?from ?to) 2))
    :condition (and 
        ...
        (at start (idle ?d))
        ; other
        (over all (train_at ?t ?to))
        (over all (<= (aside_distance ?t)(astack_distance ?to)))
    )
    :effect (and 
...
(:durative-action walk_and_enter_bside
    :parameters (
        ?d - driver
        ?t - train
        ?from - track
    ...
    :condition (and 
        ...
        ; other
        (over all (train_at ?t ?to))
        (over all (>= (+ (train_length ?t)(aside_distance ?t)) (bstack_distance ?to)))
    )
    :effect (and 
        (at start (not (driver_at ?d ?from)))
        (at start (not (idle ?d)))
