...
(:action move_aside
    ...
    :effect (and 
        (when (not (direction_bside ?t)) (increase (total-cost) 2))
...
(:action walk_and_enter_and_move_aside
    :parameters (
        ?d - driver
        ?t - train
        ?driverFrom - track
    ...
    :effect (and 
        (increase (total-cost) (+ (walking_duration ?driverFrom ?from) 3))
        (not (driver_at ?d ?driverFrom))
        (not (idle ?d))
        (driving ?d ?t)
        (operated ?t)
        ; direction
...
(:action move_bside
    ...
    :effect (and 
        (when (not (direction_aside ?t)) (increase (total-cost) 2))
...
(:action walk_and_enter_and_move_bside
    :parameters (
        ?d - driver
        ?t - train
        ?driverFrom - track
